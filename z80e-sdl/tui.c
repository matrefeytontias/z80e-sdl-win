...
debugger_state_t *tui_new_state(struct debugger_state *state, const char *title) {
	debugger_state_t *stat = (debugger_state_t*)calloc(sizeof(debugger_state_t), 1);
...
uint8_t tui_disassemble_read(struct disassemble_memory *state, uint16_t mem) {
	struct tui_disasm *disasm = (tui_disasm*)state->extra_data;
...
void tui_tick(tui_state_t *state) {
	...
	while (1) {
		...
		char result[16384];
		printf(prompt_buffer);
		fgets(result, 16384, stdin);
		char *strEnd = strchr(result, '\n');
		if (strEnd) {
			*strEnd = '\0';
		}

		if (*result == '\0' && *lastCommand != '\0') {
			memcpy(result, lastCommand, strlen(lastCommand) + 1);
		}
		...
		memcpy(lastCommand, result, strlen(result) + 1);
	}
}
