...
uint16_t command_on_register_hook(void *state, registers reg, uint16_t value) {
	on_state_t *data = (on_state_t*)state;
...
int command_on(struct debugger_state *state, int argc, char **argv) {
	...
	on_state_t *sta = (on_state_t*)malloc(sizeof(on_state_t));
	sta->deb_sta = state->create_new_state(state, argv[4]);
	sta->exec_string = (char*)malloc(strlen(argv[4]) + 1);
	...
	if (strncasecmp(argv[1], "register", 8) == 0) {
		...
		if (thing & READ) {
			hook_add_register_read(state->asic->hook, (registers)sta->look_for, sta, command_on_register_hook);
		}
		if (thing & WRITE) {
			hook_add_register_write(state->asic->hook, (registers)sta->look_for, sta, command_on_register_hook);
		}
...
int command_break(struct debugger_state *state, int argc, char **argv) {
	...
	struct break_data *data = (break_data*)malloc(sizeof(struct break_data));
...
uint8_t step_over_read_byte(struct disassemble_memory *dmem, uint16_t mem) {
	command_step_over_dism_extra_t *extra = (command_step_over_dism_extra_t*)dmem->extra_data;
	return ti_read_byte(extra->mmu, mem);
}

int step_over_disasm_write(struct disassemble_memory *mem, const char *thing, ...) {
	command_step_over_dism_extra_t *extra = (command_step_over_dism_extra_t*)mem->extra_data;
