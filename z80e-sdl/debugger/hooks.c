...
hook_info_t *create_hook_set(asic_t *asic) {
	hook_info_t *info = (hook_info_t*)malloc(sizeof(hook_info_t));
	info->cpu = asic->cpu;
	info->mmu = asic->mmu;
	...
	info->on_memory_read = (hook_memory_array_t*)malloc(sizeof(hook_memory_array_t));
	info->on_memory_read->capacity = 10;
	info->on_memory_read->callbacks = (memory_hook_callback_t*)calloc(10, sizeof(memory_hook_callback_t));

	info->on_memory_write = (hook_memory_array_t*)malloc(sizeof(hook_memory_array_t));
	info->on_memory_write->capacity = 10;
	info->on_memory_write->callbacks = (memory_hook_callback_t*)calloc(10, sizeof(memory_hook_callback_t));

	info->on_register_read = (hook_register_array_t*)malloc(sizeof(hook_register_array_t));
	info->on_register_read->capacity = 10;
	info->on_register_read->callbacks = (register_hook_callback_t*)calloc(10, sizeof(register_hook_callback_t));

	info->on_register_write = (hook_register_array_t*)malloc(sizeof(hook_register_array_t));
	info->on_register_write->capacity = 10;
	info->on_register_write->callbacks = (register_hook_callback_t*)calloc(10, sizeof(register_hook_callback_t));

	info->on_before_execution = (hook_execution_array_t*)malloc(sizeof(hook_execution_array_t));
	info->on_before_execution->capacity = 10;
	info->on_before_execution->callbacks = (execution_hook_callback_t*)calloc(10, sizeof(execution_hook_callback_t));

	info->on_after_execution = (hook_execution_array_t*)malloc(sizeof(hook_execution_array_t));
	info->on_after_execution->capacity = 10;
	info->on_after_execution->callbacks = (execution_hook_callback_t*)calloc(10, sizeof(execution_hook_callback_t));

	info->on_lcd_update = (hook_lcd_update_array_t*)malloc(sizeof(hook_lcd_update_array_t));
	info->on_lcd_update->capacity = 10;
	info->on_lcd_update->callbacks = (lcd_update_callback_t*)calloc(10, sizeof(lcd_update_callback_t));
...
int hook_add_to_memory_array(hook_memory_array_t *hook, uint16_t address_start, uint16_t address_end, void *data, hook_memory_callback callback) {
	...
	for (; x < hook->capacity; x++) {
		...
		if (x == hook->capacity - 1) {
			...
			hook->callbacks = (memory_hook_callback_t*)n;
			memset(hook->callbacks + sizeof(memory_hook_callback_t) * (hook->capacity - 10), 0, sizeof(memory_hook_callback_t) * 10);
		}
	}
...
int hook_add_to_register_array(hook_register_array_t *hook, registers flags, void *data, hook_register_callback callback) {
	...
	for (; x < hook->capacity; x++) {
		...
		if (x == hook->capacity - 1) {
			...
			hook->callbacks = (register_hook_callback_t*)n;
			memset(hook->callbacks + sizeof(register_hook_callback_t) * (hook->capacity - 10), 0, sizeof(memory_hook_callback_t) * 10);
		}
	}
...
int hook_add_to_execution_array(hook_execution_array_t *hook, void *data, hook_execution_callback callback) {
	...
	for (; x < hook->capacity; x++) {
		...
		if (x == hook->capacity - 1) {
			...
			hook->callbacks = (execution_hook_callback_t*)n;
...
int hook_add_lcd_update(hook_info_t *info, void *data, hook_lcd_update_callback callback) {
	...
	for (; x < hook->capacity; x++) {
		...
		if (x == hook->capacity - 1) {
			...
			hook->callbacks = (lcd_update_callback_t*)n;
			memset(hook->callbacks + sizeof(lcd_update_callback_t) * (hook->capacity - 10), 0, sizeof(lcd_update_callback_t) * 10);
		}
	}
