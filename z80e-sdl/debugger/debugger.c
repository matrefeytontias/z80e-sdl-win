...
int debugger_source_rc(debugger_state_t *state, const char *rc_name) {
	...
	PWSTR env_holder;
	if(SHGetKnownFolderPath(FOLDERID_LocalAppData, 0, NULL, &env_holder) != S_OK) {
		CoTaskMemFree(env_holder);
		free(env);
		printf("Couldn't find required variable.\nAborting.\n");
		exit(1);
	} else {
		// Wide Char To w/e i.e char*
		wcstombs(env, env_holder, 512);
		printf("Var found : %s\n", env);
	}
#endif
	char *realloced;
	...
	if (!env) {
		...
		realloced = (char*)malloc(hsize + 9);
		strcpy(realloced, env);
		strcpy(realloced + hsize, "/.config");
		strsize = hsize + 8;
	} else {
		strsize = strlen(env);
		realloced = (char*)malloc(strsize + 1);
		strcpy(realloced, env);
	}

	realloced = (char*)realloc(realloced, strsize + strlen(rc_name) + 2);
#ifndef WIN32
...
debugger_t *init_debugger(asic_t *asic) {
	debugger_t *debugger = (debugger_t*)calloc(1, sizeof(debugger_t));

	debugger->commands.count = default_command_count;
	debugger->commands.capacity = default_command_count + 10;
	debugger->commands.commands = (debugger_command_t**)calloc(sizeof(debugger_command_t *), debugger->commands.capacity);
	int i = 0;
...
void register_command(debugger_t *debugger, const char *name, debugger_function_t function, void *state, int priority) {
	...
	debugger_command_t *command = (debugger_command_t*)malloc(sizeof(debugger_command_t));
	command->name = name;
	...
	if (list->count >= list->capacity) {
		list->commands = (debugger_command_t**)realloc(list->commands, sizeof(debugger_command_t *) * (list->capacity + 10));
		list->capacity += 10;
	}
...
char **debugger_parse_commandline(const char *cmdline, int *argc) {
	...
	while (*cmdline != 0 && *cmdline != '\n') {
		...
		buffer[buffer_pos] = (char*)malloc(length + 1);
	...
	char **result = (char**)malloc((buffer_pos + 1) * sizeof(char *));
