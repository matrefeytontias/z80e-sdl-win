...
z80cpu_t* cpu_init(log_t *log) {
	z80cpu_t *cpu = (z80cpu_t*)calloc(1, sizeof(z80cpu_t));
	cpu->log = log;
...
void execute_bli(int y, int z, struct ExecutionContext *context) {
	...
	case 4:
		...
		case 1: // CPI
			context->cycles += 12;
			newB = cpu_read_byte(context->cpu, r->HL++);
			aminushl = r->A - newB;
			r->F = _flag_sign_8(aminushl) | _flag_zero(aminushl)
				| _flag_halfcarry_8_sub(r->A, newB)
			...
			case 1:
				...
				case 0:
					if (context.y == 6) { // IN (C)
						...
						if (ioDevice.read_in != NULL) {
							newB = ioDevice.read_in(ioDevice.device);
							r->F = _flag_sign_8(newB) | _flag_undef_8(newB)
								| __flag_c(r->flags.C) | _flag_zero(newB)
								| _flag_parity(newB);
						}
					} else { // IN r[y], (C)
						context.cycles += 8;
						...
						if (ioDevice.read_in != NULL) {
							...
							write_r(context.y, newB, &context);
							r->F = _flag_sign_8(newB) | _flag_undef_8(newB)
								| __flag_c(r->flags.C) | _flag_zero(newB)
								| _flag_parity(newB);
						}
				...
				case 4: // NEG
					...
					r->F = _flag_sign_8(r->A) | _flag_zero(r->A)
						...
						| _flag_halfcarry_8_sub(old, (old - newB) & 0xff);
					break;
				case 5:
					if (context.y == 1) { // RETI
			...
			case 0:
				...
				case 1:
					...
					case 1: // ADD HL, rp[p]
						...
						newB16 = HLorIw(&context, old16 + op16);
				...
				case 4: // INC r[y]
					...
					newB = write_r(context.y, old + 1, &context);
					r->F = __flag_c(r->flags.C) | _flag_sign_8(newB) | _flag_zero(newB)
				...
				case 5: // DEC r[y]
					...
					r->F = __flag_c(r->flags.C) | _flag_sign_8(newB) | _flag_zero(newB)
						...
						| _flag_subtract(1) | _flag_undef_8(newB);
					break;
				case 6: // LD r[y], n
					context.cycles += 7;
