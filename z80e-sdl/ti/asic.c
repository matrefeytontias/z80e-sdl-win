...
uint8_t read_unimplemented_port(void *device) {
	unimplemented_device_t *d = (unimplemented_device_t*)device;
...
void plug_devices(asic_t *asic) {
	...
	int i;
	for (i = 0; i < 0x100; i++) {
		unimplemented_device_t *d = (unimplemented_device_t*)malloc(sizeof(unimplemented_device_t));
		d->asic = asic;
		d->port = i;
...
asic_t *asic_init(ti_device_type type, log_t *log) {
	asic_t* device = (asic_t*)malloc(sizeof(asic_t));
	device->log = log;
	...
	device->timers = (z80_hardware_timers_t*)calloc(sizeof(z80_hardware_timers_t), 1);
	device->timers->max_timers = 20;
	device->timers->timers = (z80_hardware_timer_t*)calloc(sizeof(z80_hardware_timer_t), 20);
...
int asic_add_timer(asic_t *asic, int flags, double frequency, timer_tick tick, void *data) {
	...
	for (i = 0; i < asic->timers->max_timers; i++) {
		...
		if (i == asic->timers->max_timers - 1) {
			...
			asic->timers->timers = (z80_hardware_timer_t*)realloc(asic->timers->timers, sizeof(z80_hardware_timer_t) * asic->timers->max_timers);
			z80_hardware_timer_t *ne = &asic->timers->timers[asic->timers->max_timers - 10];
			memset(ne, 0, sizeof(z80_hardware_timer_t) * 10);
		}
	}

	timer->cycles_until_tick = (int)(asic->clock_rate / frequency);
	...
	for (i = 0; i < asic->timers->max_timers; i++) {
		...
		if (timer->flags & TIMER_IN_USE) {
			timer->cycles_until_tick =
				(int)(new_rate / (timer->cycles_until_tick * timer->frequency));
		}
	}
