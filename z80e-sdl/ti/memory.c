...
ti_mmu_t* ti_mmu_init(ti_device_type device_type) {
	ti_mmu_t *mmu = (ti_mmu_t*)malloc(sizeof(ti_mmu_t));
	switch (device_type) {
	case TI83p:
	case TI73:
	...
	mmu->ram = (uint8_t*)malloc(mmu->settings.ram_pages * 0x4000);
	memset(mmu->ram, 0, mmu->settings.ram_pages * 0x4000);
	mmu->flash = (uint8_t*)malloc(mmu->settings.flash_pages * 0x4000);
